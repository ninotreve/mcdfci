...
class Determinant
{
    public:
        ...
        // Return the number of 1's in repr
        int popcount() const
        {
            int result = 0;
            for (auto t = 0; t < size; ++t)
...
template<int N = 1>
Determinant<N> get_and_det(const Determinant<N> &det1, const Determinant<N> &det2)
{
    Determinant<N> new_det;
    for (auto t = 0; t < N; ++t)
    {
        new_det.repr[t] = det1.repr[t] & det2.repr[t];
    }
    return new_det;
}

template<int N = 1>
Determinant<N> get_xor_det(const Determinant<N> &det1, const Determinant<N> &det2)
{
    Determinant<N> new_det;
    for (auto t = 0; t < N; ++t)
    {
...
template<int N = 1>
std::ostream& operator<<(std::ostream& os, const Determinant<N>& det) {
    os << std::hex;
    for (auto t = 0; t < N; ++t)
    {
        os << det.repr[t];
