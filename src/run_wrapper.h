...
double run_cdfci(Option& option, Fcidump& fci)
{
    ...
    CoordinatePick<N> *coord_pick;
    CoordinateUpdate<N> *coord_update;
    if (option["solver"]["cdfci"]["coordinate_pick"] == "gcd_grad")
        coord_pick = new CoordinatePickGcdGrad<N>();
    else
    {
        std::cout << "Error: the coordinate pick method "
            << option["solver"]["cdfci"]["coordinate_pick"]
            << " is not implemented." << std::endl;
        exit(-1);
    }

    if (option["solver"]["cdfci"]["coordinate_update"] == "ls")
        coord_update = new CoordinateUpdateLS<N>();
    else if (option["solver"]["cdfci"]["coordinate_update"] == "eig")
        coord_update = new CoordinateUpdateEig<N>();
    else
    {
        std::cout << "Error: the coordinate update method "
            << option["solver"]["cdfci"]["coordinate_update"]
            << " is not implemented." << std::endl;
        exit(-1);
    }
    ...
    // Step 3: Solve.
    int k = option["solver"]["cdfci"]["num_of_coordinate"];
#ifdef _OPENMP
    if (k > omp_get_num_procs())
    {
        std::cout << "Warning: Not enough processes. Set k = " <<
            omp_get_num_procs() << std::endl;
        k = omp_get_num_procs();
    }
#endif
    int m = option["solver"]["cdfci"]["estimate"];
    cdfci_solver.solve(h, *coord_pick, *coord_update, k, m);
